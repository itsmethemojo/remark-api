...
@test "query bookmarks should be empty" {
  result="$(curl -s -X GET -H "${TEST_USER_HEADER_PREFIX}1" ${API_URI}/api/v1/bookmark/)"
  [ "$(echo $result | jq -c)" == '{"Bookmarks":[],"Remarks":[],"Clicks":[]}' ]
}

@test "add bookmark X" {
  result="$(curl -s -X POST -H "${TEST_USER_HEADER_PREFIX}1" ${API_URI}/api/v1/bookmark/remark/ -d 'url=https://github.com')"
...
@test "click bookmark X" {
  result="$(curl -s -X POST -H "${TEST_USER_HEADER_PREFIX}1" ${API_URI}/api/v1/bookmark/click/ -d id=$(curl -s -X GET -H "${TEST_USER_HEADER_PREFIX}1" ${API_URI}/api/v1/bookmark/ | jq -r .Bookmarks[0].ID))"
  [ "$(echo $result | jq -c)" == '{"message":"ok"}' ]
}
...
@test "add bookmark Y" {
  result="$(curl -s -X POST -H "${TEST_USER_HEADER_PREFIX}1" ${API_URI}/api/v1/bookmark/remark/ -d url=${API_URI}/swagger/doc.json)"
  [ "$(echo $result | jq -c)" == '{"message":"ok"}' ]
}
...
@test "change title of bookmark Y" {
  result="$(curl -s -X POST -H "${TEST_USER_HEADER_PREFIX}1" ${API_URI}/api/v1/bookmark/$(curl -s -X GET -H "${TEST_USER_HEADER_PREFIX}1" ${API_URI}/api/v1/bookmark/ | jq -r .Bookmarks[1].ID)/ -d 'title=ä new fancy title')"
  [ "$(echo $result | jq -c)" == '{"message":"ok"}' ]
}
...
@test "click bookmark Y" {
  result="$(curl -s -X POST -H "${TEST_USER_HEADER_PREFIX}1" ${API_URI}/api/v1/bookmark/click/ -d id=$(curl -s -X GET -H "${TEST_USER_HEADER_PREFIX}1" ${API_URI}/api/v1/bookmark/ | jq -r .Bookmarks[1].ID))"
  [ "$(echo $result | jq -c)" == '{"message":"ok"}' ]
}
...
@test "delete bookmark X" {
  result="$(curl -s -X DELETE -H "${TEST_USER_HEADER_PREFIX}1" ${API_URI}/api/v1/bookmark/$(curl -s -X GET -H "${TEST_USER_HEADER_PREFIX}1" ${API_URI}/api/v1/bookmark/ | jq -r .Bookmarks[0].ID)/)"
  [ "$(echo $result | jq -c)" == '{"message":"ok"}' ]
}
...
@test "add bookmark Y but with user 2" {
  result="$(curl -s -X POST -H "${TEST_USER_HEADER_PREFIX}2" ${API_URI}/api/v1/bookmark/remark/ -d url=${API_URI}/swagger/doc.json)"
  [ "$(echo $result | jq -c)" == '{"message":"ok"}' ]
}

@test "query bookmarks should have bookmark Y added for user 2" {
  result="$(curl -s -X GET -H "${TEST_USER_HEADER_PREFIX}2" ${API_URI}/api/v1/bookmark/)"
  [ "$(echo $result | jq -r .Bookmarks[0].Url)" == "${API_URI}/swagger/doc.json" ]
...
@test "user 3 is unauthorized due to not in allowed users list" {
   result="$(curl -s -X GET -H "${TEST_USER_HEADER_PREFIX}3" ${API_URI}/api/v1/bookmark/)"
   [ "$(echo $result | jq -c)" == '{"message":"Unauthorized"}' ]
}